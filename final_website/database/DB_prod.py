...
con = sqlite3.connect('DB.db') # creates a DB.db if one is not found
cur = con.cursor() 
...
with open('./Data/main.csv') as file: # commit a row in the SQL table for every row in the file
    for row in file:
        cur.execute("INSERT INTO Variants VALUES(?,?,?,?,?,?)", row.split(',')) # takes into account the files are csv
        con.commit()
...
with open('./Data/variant_gene_pairings.csv') as file:
    for row in file:
        ...
        gene_name = row.split(',')[1].strip()  # strip() removes leading/trailing whitespace which was present in this csv
        # Check if the record already exists
        cur.execute("SELECT * FROM Variant_gene_relationship WHERE Variant_name=? AND Gene_name=?", (row.split(',')[0], gene_name))
        existing_record = cur.fetchone()

        # Insert the record only if it doesn't already exist as there were issues with duplicated entries
...
cur.execute("CREATE TABLE chr6_GBR_r(rs_value VARCHAR(30) PRIMARY KEY);") # start by creating empty table
...
with open('./Data/chr6_GBR_r.csv') as file: # then add the data to the columns
    for row in file:
...
con.close() # must close connection!